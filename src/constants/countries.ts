/**
 * Single source of truth for the app's country data.
 *
 * `COUNTRY_CODES` carries the dial code, ISO-3166 alpha-2 code, and display
 * name for every country. It powers:
 *   - the phone-number calling-code picker (`PhoneInput`), which needs `code`
 *   - the country pickers on Edit Profile / Apple name capture, which use the
 *     derived `COUNTRIES` list (`{ label, value }`, value = ISO2)
 *
 * Add or edit countries here only — do not re-declare a list elsewhere.
 */
export interface CountryCode {
  /** E.164 dial code, e.g. '+61' */
  code: string;
  /** ISO-3166 alpha-2 country code, e.g. 'AU' */
  country: string;
  /** Display name */
  label: string;
}
...
/**
 * `{ label, value }` list for country pickers where the stored value is the
 * ISO-3166 alpha-2 code (Edit Profile, Apple name capture). Derived from
 * `COUNTRY_CODES` so there is one list to maintain.
 */
export const COUNTRIES = COUNTRY_CODES.map((c) => ({
  label: c.label,
  value: c.country,
}));
