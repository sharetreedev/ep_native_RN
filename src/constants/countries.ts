/**
 * Canonical country list — the single source of truth for both:
 *   - the phone-prefix picker (needs the E.164 dial `code`), and
 *   - the country-of-residence picker (needs `label` + ISO `country`).
 *
 * Keep this list complete; screens derive their shapes from it rather than
...
/**
 * Country-of-residence options ({ label, value: ISO code }) for the profile /
 * signup country picker. Derived from {@link COUNTRY_CODES} so the two lists
 * never drift apart.
 */
export const COUNTRIES = COUNTRY_CODES.map(({ label, country }) => ({
  label,
  value: country,
}));
